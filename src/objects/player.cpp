...
Vec4 *Player::getNextPosition(float deltaTime, const Pad &t_pad, const Camera &t_camera)
{
    Vec4 *result = new Vec4(mesh.position);
...
void Player::updateGravity(float deltaTime)
{
    ...
    Vec4 newYPosition = mesh.position - (this->velocity * deltaTime);
...
void Player::checkIfWillCollideBlock(Block *t_blocks[], int blocks_ammount)
{
    ...
    // Get the direction
    Vec4 rayDir = *nextPlayerPos - this->mesh.position;
    rayDir.normalize();
