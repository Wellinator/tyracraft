...
void Cow::fixedUpdate(const float& fixedDeltaTime) {
  // Reset lerp state
...
void Cow::update(const float& deltaTime) {
...
Vec4 Cow::getNextXZPosition(const float& fixedDeltaTime, const Vec4& target) {
...
bool Cow::updateXZPosition(const float& deltaTime, const Vec4& nextPosition,
                           u8 isColliding) {
...
void Cow::resolveOutOfWorldBoundaries() {
  velocity = Vec4(0.0f, 0.0f, 0.0f);
  shouldUnspawn = true;
  return;
};

const float Cow::getHeight() { return DOUBLE_BLOCK_SIZE * 0.9F; }
...
void Cow::setWalkingAnimation() { setAnimation(WALK_ANIMATION); }

void Cow::setIdleAnimation() { setAnimation(IDLE_ANIMATION); }

void Cow::updateStateInWater() {
...
bool Cow::isOnWater() { return _isOnWater; }

bool Cow::isUnderWater() { return _isUnderWater; }

void Cow::playStepSfx() {
...
void Cow::onMoved() {
  if (lastTimePlayedStepSfx > stepSfxLimit) {
    ...
    if (getCurrentAnimation().animationId != WALK_ANIMATION) {
      setWalkingAnimation();
    }
...
void Cow::onStopMoving() { setIdleAnimation(); }

// ----
// Override
// ----
/** Mob category */
MobCategory Cow::getCategory() { return MobCategory::Passive; }

/** Mob type */
MobType Cow::getType() { return MobType::Cow; }
