...
SplashScreen::SplashScreen(Renderer* t_renderer) {
  ...
  std::string tyracraftSplashBasePath = FileUtils::fromCwd("assets/splash_screen/tyracraft/");
  std::string tyraSplashBasePath = FileUtils::fromCwd("assets/splash_screen/tyra/");
...
void SplashScreen::render() {
  ...
  if (!hasShowedTyraCraft) {
    renderTyraCraftSplash(this->t_renderer);
    return;
  }
  if (!hasShowedTyra) {
    renderTyraSplash(this->t_renderer);
    return;
  }
}

void SplashScreen::renderTyraSplash(Renderer* renderer) {
...
void SplashScreen::renderTyraCraftSplash(Renderer* renderer) {
  for (u8 index = 0; index < 16; index++) {
