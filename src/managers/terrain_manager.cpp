...
void TerrainManager::updateChunkByPlayerPosition(Player *player)
{
    if (this->lastPlayerPosition == NULL)
        this->lastPlayerPosition = new Vec4();
...
void TerrainManager::buildChunk(int offsetX, int offsetY, int offsetZ)
{
    ...
    for (int z = -HALF_CHUNCK_SIZE; z < HALF_CHUNCK_SIZE; z++)
    {
        for (int x = -HALF_CHUNCK_SIZE; x < HALF_CHUNCK_SIZE; x++)
        {
            ...
            {
                Vec4 *tempBlockOffset = new Vec4(offsetX + x,
                                                       offsetY + y,
                                                       offsetZ + z);
...
void TerrainManager::getTargetBlock(const Vec4 &playerPosition, Camera *t_camera)
{
...
void TerrainManager::putBlock(u8 blockToPlace)
{
    ...
    // Detect face
    const BBox *blockCenterPos = this->targetBlock->mesh.getCurrentBoundingBox();
...
{
    return new Vec4((worldPosition->x == 0 ? 0 : std::trunc((worldPosition->x + DUBLE_BLOCK_SIZE) / DUBLE_BLOCK_SIZE) * DUBLE_BLOCK_SIZE),
