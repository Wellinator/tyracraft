...
void BlockManager::loadBlocks() {
  // TODO: Refactor to MinecraftPipeline
  ...
  //   sprucePlanksBlock.loadObj(MODELS_PATH, "spruce_planks", BLOCK_SIZE,
  //   true); birchPlanksBlock.loadObj(MODELS_PATH, "birch_planks", BLOCK_SIZE,
  //   true); acaciaPlanksBlock.loadObj(MODELS_PATH, "acacia_planks",
  //   BLOCK_SIZE, true);

  //   // Stone Bricks
  //   stoneBrickBlock.loadObj(MODELS_PATH, "stone_bricks", BLOCK_SIZE, true);
  //   crackedStoneBricksBlock.loadObj(MODELS_PATH, "cracked_stone_bricks",
  //                                   BLOCK_SIZE, true);
  //   mossyStoneBricksBlock.loadObj(MODELS_PATH, "mossy_stone_bricks",
  //   BLOCK_SIZE,
  //                                 true);
  //   chiseledStoneBricksBlock.loadObj(MODELS_PATH, "chiseled_stone_bricks",
  //                                    BLOCK_SIZE, true);

  //   // Woods
  //   strippedOakWoodBlock.loadObj(MODELS_PATH, "stripped_oak_log", BLOCK_SIZE,
  //                                true);
...
void BlockManager::linkTextureByBlockType(u8 blockType, const u32 t_meshId,
                                          u8 materialIndex) {
  this->t_renderer->core.texture.repository
      .getBySpriteOrMesh(
          getMeshByBlockType(blockType)->getMaterial(materialIndex)->getId())
      ->addLink(t_meshId);
}

void BlockManager::removeTextureLinkByBlockType(u8 blockType,
                                                const u32 t_meshId,
                                                u8 materialIndex) {
  this->t_renderer->core.texture.repository
      .getBySpriteOrMesh(
          getMeshByBlockType(blockType)->getMaterial(materialIndex)->getId())
      ->removeLinkById(t_meshId);
}
...
void BlockManager::pushItems() {
  ...
  //       new BlockItem(&diamondOreBlock, DIAMOND_ORE_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&redsoneOreBlock, REDSTONE_ORE_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&emeraldOreBlock, EMERALD_ORE_BLOCK));

  //   // Wood Planks
  //   this->blockItems.push_back(new BlockItem(&oakPlanksBlock,
  //   OAK_PLANKS_BLOCK)); this->blockItems.push_back(
  //       new BlockItem(&sprucePlanksBlock, SPRUCE_PLANKS_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&birchPlanksBlock, BIRCH_PLANKS_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&acaciaPlanksBlock, ACACIA_PLANKS_BLOCK));

  //   // Stone bricks
  //   this->blockItems.push_back(
  //       new BlockItem(&stoneBrickBlock, STONE_BRICK_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&crackedStoneBricksBlock, CRACKED_STONE_BRICKS_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&mossyStoneBricksBlock, MOSSY_STONE_BRICKS_BLOCK));
  //   this->blockItems.push_back(
  //       new BlockItem(&chiseledStoneBricksBlock,
  //       CHISELED_STONE_BRICKS_BLOCK));

  //   // Woods
  //   this->blockItems.push_back(
  //       new BlockItem(&strippedOakWoodBlock, STRIPPED_OAK_WOOD_BLOCK));
  //   this->blockItems.push_back(new BlockItem(&oakLogBlock, OAK_LOG_BLOCK));
  //   this->blockItems.push_back(new BlockItem(&oakLeavesBlock,
  //   OAK_LEAVES_BLOCK));
}

Mesh* BlockManager::getMeshByBlockType(u8 blockType) {
  for (int i = 0; i < blockItems.size(); i++)
    if (blockItems[i]->blockId == blockType) return blockItems[i]->t_mesh;
  return NULL;
}