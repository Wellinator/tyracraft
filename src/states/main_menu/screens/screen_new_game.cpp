...
void ScreenNewGame::render() {
  ...
  this->context->fontManager.printText(
  ...
  switch (this->model.type) {
    case WorldType::WORLD_TYPE_ORIGINAL:
      this->context->fontManager.printText("World Type: Original", 130, 277);
      break;
    case WorldType::WORLD_TYPE_FLAT:
      this->context->fontManager.printText("World Type: Flat", 150, 277);
      break;
    case WorldType::WORLD_TYPE_ISLAND:
      this->context->fontManager.printText("World Type: Island", 135, 277);
      break;
    case WorldType::WORLD_TYPE_WOODS:
      this->context->fontManager.printText("World Type: Woods", 140, 277);
      break;
    case WorldType::WORLD_TYPE_FLOATING:
      this->context->fontManager.printText("World Type: Floating", 130, 277);
      break;
  ...
  this->context->fontManager.printText("Create New World", 145, 322);

  if (isEditingSeed) {
    this->t_renderer->renderer2D.render(&btnTriangle);
    this->t_renderer->renderer2D.render(&btnCross);

    this->context->fontManager.printText("Confirm", 35, 407);
    this->context->fontManager.printText("Cancel", 160, 407);
  } else {
    if (activeOption == ScreenNewGameOptions::TexturePack) {
      ...
      this->context->fontManager.printText("Prev", 35, 407);
      this->context->fontManager.printText("Next", 160, 407);
      this->context->fontManager.printText("Back", 280, 407);
    } else {
      this->t_renderer->renderer2D.render(&btnTriangle);
      this->t_renderer->renderer2D.render(&btnCross);

      if (activeOption == ScreenNewGameOptions::Seed) {
        this->context->fontManager.printText("Edit", 35, 407);
        this->context->fontManager.printText("Random", 280, 407);
        this->t_renderer->renderer2D.render(&btnCircle);
      } else {
        this->context->fontManager.printText("Select", 35, 407);
      }
      this->context->fontManager.printText("Back", 160, 407);
    }
  }
}
...
void ScreenNewGame::handleSeedInput() {
  auto clickedButtons = this->context->context->t_engine->pad.getClicked();
  auto validChars = this->context->fontManager.NumericValidChars;
